...
const WhatsappScan = () => {
  ...
  const checkDeviceStatus = async (id) => {
    try {
      const res = await fetch(`http://localhost:5000/get-device?deviceId=${id}`);
      if (!res.ok) {
...
await fetch(`http://localhost:5000/create-device?deviceId=${id}`);
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const res = await fetch(`http://localhost:5000/get-qr?deviceId=${activeDevice}`);
        if (!res.ok) {
  ...
  const deleteDevice = async (id) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/delete-device?deviceId=${id}`);
      const data = await res.json();
  ...
  const disconnectDevice = async (id) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/logout?deviceId=${id}`);
      const data = await res.json();
